...
var ServiceNowSync = (function() {
          ...
          _this.getRecord(folderSettings, sys_id, record => {
            if (record) {
              ...
              if (diff.length > 1 || diff[0].added || diff[0].removed) {
                ...
                vscode.window
                  .showInformationMessage('Remote record has been updated, overwrite?', 'Yes', 'No')
                  .then(function(res) {
                    if (res === 'Yes') {
      ...
      _this.getRecord(folderSettings, sys_id, record => {
        if (record) {
          ...
          if (diff.length > 1 || diff[0].added || diff[0].removed) {
            // Please note this is a variadic function
            vscode.window
              .showInformationMessage('Remote record has been updated, overwrite local copy?', 'Yes', 'No')
              .then(function(res) {
                if (res === 'Yes') {
  ...
  ServiceNowSync.prototype.pullFile = function(selectedFolder, query) {
    ...
    function displayRecordConfirmation(result) {
      if (result) {
        vscode.window
          .showInformationMessage('Action will create or update ' + result.length + ' files, continue?', 'Yes', 'No')
          .then(function(res) {
            if (res === 'Yes') {
    ...
    function createFile(record) {
      if (typeof settings.multi !== 'undefined' && settings.multi === true) {
        _.each(subSettings, setting => {
          saveFileAndUpdateSettings(record, setting, setting.folder);
        });
      } else {
        saveFileAndUpdateSettings(record, settings, folder);
      }
    }

    function saveFileAndUpdateSettings(record, settings, folder) {
      let fileName = (record[settings.display] + '.' + settings.extension).replace(/[<>:"\\\/|?*]/g, '_');
